...
> **Note on vLLM:** vLLM does not support macOS. All exercises in this solution use the **Google Gemini API** (mainly `gemini-3.1-flash-lite-preview`) via its OpenAI-compatible endpoint, accessed with the standard OpenAI Python SDK. The API interface is identical — only the `api_key` and `base_url` differ from a local vLLM setup.
